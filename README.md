...
3. We go into this class, inside the Master Function

4. We look for the appropriate method for this item
...
3. After that we initialize it

5. Lastly we just need to make it visible and also open the inventory of the Player if it wasn't opened already
...
3. The final location which is generated by the ${\color{orange}Find \ Location \ For \ Item \ Drop}$ method inside the [BFL functions](#bfl) blueprint

5. Middle point between the first two locations
...
3. Following that, we need to find out where the ground is - this is done with a simple trace from the origin of the actor who threw the item to -9999 on the Z axis
	* You may need to change this value to 9999 if your world's Z axis is flipped for some reason

4. The middle point between the ground and the start of that trace + the height is where our main box trace starts

6. We generate a random distance that we want to throw the item to

8. Using that distance and the Random Unit Vector, we can calculate where our box trace ends

10. And finally, if that box traces does not hit anything, that is the location we return

12. Otherwise, we return the location directly down from where the item spawned
...
3. Copy over those files from the Engine/Plugin folder into your 'Plugins' folder

5. Load your project and go to the 'Edit' Tab

7. Click Plugins and search for 'CompleteInventory'

9. Enable the Plugin, restart the editor and compile everything
...
3. Add **three** new mappings and assign them IA_SplitItem, IA_HalveStack and IA_OpenInventory

5. Assign some different keys if you do not like the preset ones
